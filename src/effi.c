...
#  define WRITE_C_STRING(x,y) write_ascstring((y),(x))
#  define WRITE_FMT_STRING(x,y,...) write_fmt_string((x),(y),__VA_ARGS__)
...
print_ffiobject(Lisp_Object obj, Lisp_Object printcharfun, int escapeflag)
{
  ...
  if (print_readably) {
    ...
    error("printing unreadable object #<ffiobject 0x%x>",
	  ffio->header.uid);
#else
    printing_unreadable_object_fmt ("#<ffiobject 0x%x>",
                                    LISP_OBJECT_UID (obj));
#endif	/* SXEMACS */
  }
  ...
  WRITE_FMT_STRING (printcharfun, "size=%ld fotype=%d foptr=%p>",
		    (long) XFIXNUM (ffio->size), ffio->fotype,
		    ffio->fop.generic);
}
...
#else
static Bytecount
sizeof_ffiobject(Lisp_Object object)
{
  const Lisp_EffiObject *effio = XEFFIO (object);
  return (sizeof (Lisp_EffiObject) + effio->storage_size);
}
#endif	/* SXEMACS */

/* Define ffiobject implementation */
struct lrecord_implementation lrecord_ffiobject = {
  .name = "ffiobject",
      ...
      (type))
{
  ...
  if (EQ(type, Q_void))
    ...
    CHECK_FIXNUM(asize);
    tsize = XFIXNUM(asize) * XFIXNUM(Fffi_size_of_type(atype));
    ...
    while (!NILP(slots)) {
      ...
      int slot_size = XFIXNUM(Fffi_size_of_type(slot_type));
      if (slot_size > tsize)
	tsize = slot_size;
      slots = XCDR(slots);
    }
      ...
      (type, size))
{
  ...
  if (CONSP (ctype) && EQ (XCAR (ctype), Q_c_data) && FIXNUMP (XCDR (ctype)))
    size = XCDR(type);

  cs_or_cd = EQ (ctype, Q_c_string) || (EQ (ctype, Q_c_data));
  if ((cs_or_cd && (XFIXNUM (size) < 1))
      || (!(cs_or_cd || FFI_POINTERP (ctype))
	  && (XFIXNUM(size) < XFIXNUM(Fffi_size_of_type(type)))))
#ifdef SXEMACS
    signal_simple_error("storage size too small to store type",
                        list2(size, type));

  ffio = alloc_lcrecord(sizeof(Lisp_EffiObject)+XFIXNUM(size),
			&lrecord_ffiobject);
  ...
  result = old_alloc_sized_lcrecord (sizeof (Lisp_EffiObject) + XFIXNUM(size),
                                     &lrecord_ffiobject);
  ffio = XEFFIO (result);
#endif	/* SXEMACS */
  ...
  ffio->storage_size = XFIXNUM(size);
      ...
      (address))
{
  ...
  if (FIXNUMP(address))
    addr = XFIXNUM(address);
  else if (FLOATP(address))
    addr = XFLOATFIXNUM(address);
  else {
#ifdef SXEMACS
...
#    define EXT ".so"
  ...
  Lisp_Object fo = Qnil, string_with_extension = Qnil;
  Lisp_EffiObject *ffio;
  Ibyte *dotpos, *constructed;
  Bytecount libnamelen;
  dll_handle *handler;
  struct gcpro gcpro1;
  ...
  do
    {
      /* Aidan Kehoe, Sa 31 Dez 2011 18:28:48 GMT; I've added this do {}
         while (0) in the course of merging trunk code into the FFI
         repository; the idea is that we look for both .dylib and .so
         versions of our files, if an extension is not supplied. */
      dotpos = qxestrrchr (XSTRING_DATA (libname), '.');
      if (NULL != dotpos)
        {
          /* File already has an extension, don't attempt to add one. */
          handler = dll_open (libname);
          if (handler != NULL)
            {
              break;
            }
      ...
      libnamelen = XSTRING_LENGTH (libname);
      constructed = alloca_ibytes (libnamelen + sizeof (EXT));
      memcpy (constructed, XSTRING_DATA (libname), libnamelen);
      /* Will copy the trailing zero; */
      memcpy (constructed + libnamelen, EXT, sizeof (EXT));
      string_with_extension = build_istring (constructed);
      GCPRO1 (string_with_extension);
      handler = dll_open (string_with_extension);
      UNGCPRO;

      if (handler != NULL)
        {
          /* We've found the library with the default extension. */
          break;
        }

      /* OK, try with the backup extension (especially relevant on OS X,
         where we moved from using dyld to using the dlsym API, but where we
         used to rely on the presence of dyld to work out whether to add
         .dylib to file names) .*/
      if (0 == strcmp (EXT, ".so"))
        {
          constructed = alloca_ibytes (libnamelen + sizeof (".dylib"));
          memcpy (constructed, XSTRING_DATA (libname), libnamelen);
          /* will copy the trailing nil; */
          memcpy (constructed + libnamelen, ".dylib", sizeof (".dylib"));
          string_with_extension = build_istring (constructed);
          GCPRO1 (string_with_extension);
          handler = dll_open (string_with_extension);
          UNGCPRO;
      ...
      else if (0 == strcmp (EXT, ".dylib"))
        {
          constructed = alloca_ibytes (libnamelen + sizeof (".so"));
          memcpy (constructed, XSTRING_DATA (libname), libnamelen);
          memcpy (constructed + libnamelen, ".so", sizeof (".so"));
          string_with_extension = build_istring (constructed);
          GCPRO1 (string_with_extension);
          handler = dll_open (string_with_extension);
          UNGCPRO;
      ...
      (type, sym))
{
  ...
  ffio = XEFFIO(fo);
  ffio->fop.ptr = dll_function (RTLD_DEFAULT, XSTRING_DATA (sym));
...
Return dl error string.
      ...
      ())
{
  const Extbyte *dles = dlerror();

  if (dles != NULL)
    {
      return build_extstring (dles, EFFI_CODING);
    }
  else
    {
      return Qnil;
      ...
      (type, sym))
{
  ...
  fo = Fmake_ffi_object(type, Qnil);
  ffio = XEFFIO(fo);
  ffio->fop.fun = dll_function (RTLD_DEFAULT, XSTRING_DATA (sym));
...
*/
      (type))
{
  return make_fixnum(ffi_type_align(type));
}
      ...
      (type, slot))
{
  ...
  if (!CONSP(type)) {
    ...
    Fsignal(Qwrong_type_argument,
	    list2(Qstringp, build_ascstring("Not struct or union")));
#endif	/* SXEMACS */
  }
  ...
  while (!NILP(slots)) {
    ...
    tmp_size = XFIXNUM(Fffi_size_of_type(tmp_slot));
    while (tmp_size > 0) {
      if (lpad < tmp_align) {
	retoff += lpad;
	lpad = align;
  ...
  return make_fixnum(retoff + lpad);
}
...
/* this function canNOT GC */
  ...
  if (EQ(type, Q_char))
    ...
    retval = make_fixnum(*(char*)ptr);
  else if (EQ(type, Q_unsigned_byte))
    retval = make_fixnum(*(unsigned char*)ptr);
  else if (EQ(type, Q_short))
    retval = make_fixnum(*(short*)ptr);
  else if (EQ(type, Q_unsigned_short))
    retval = make_fixnum(*(unsigned short*)ptr);
  else if (EQ(type, Q_int))
    retval = make_fixnum(*(int*)ptr);
  else if (EQ(type, Q_unsigned_int))
    retval = make_fixnum(*(unsigned int*)ptr);
  else if (EQ(type, Q_long))
    retval = make_fixnum(*(long*)ptr);
  else if (EQ(type, Q_unsigned_long))
    retval = make_fixnum(*(unsigned long*)ptr);
  else if (EQ(type, Q_float))
    retval = make_float(*(float*)ptr);
  else if (EQ(type, Q_double))
    retval = make_float(*(double*)ptr);
  else if (EQ(type, Q_c_string)) {
    retval = build_extstring((char*)ptr, Qbinary);
      ...
      (fo, offset, type))
{
  ...
  ffio = XEFFIO(fo);
  ptr = (void*)((char*)ffio->fop.ptr + XFIXNUM(offset));
  ...
  if (EQ(retval, Qnone)) {
    ...
    if (EQ(type, Q_c_data) ||
        (CONSP(type) && EQ(XCAR(type), Q_c_data)))
      {
        size_t tlen;
        if (EQ(type, Q_c_data)) {
          tlen = ffio->storage_size - XFIXNUM(offset);
        } else {
          CHECK_FIXNUM(XCDR(type));
          tlen = XUINT(XCDR(type));
        }
        
        retval = make_extstring(ptr, tlen, Qbinary);
      } else {
#ifdef SXEMACS
      ...
      (farray, idx))
{
  ...
  return Fffi_fetch(farray,
		    make_fixnum(XFIXNUM(Fffi_size_of_type(type)) * XFIXNUM(idx)),
		    type);
}
    ...
    *(double*)ptr = XFLOAT_DATA(val);
    ...
    memcpy((char*)ptr,
	   ...
	   XSTRING_LENGTH(val) + 1);
	     ...
	     (CONSP(val_type) &&
	      EQ(XCAR(val_type), Q_c_data) && FIXNUMP(XCDR(val_type)))) {
    char *val_ext;
    ...
    if (CONSP (val_type) && (val_ext_len > XFIXNUM (XCDR (val_type))))
      {
#ifdef SXEMACS
      ...
      Fsignal(Qrange_error,
	      list2(Qstringp,
		    build_ascstring("storage size too small")));
#endif	/* SXEMACS */
    }
...
#else
 	  Fsignal (Qwrong_type_argument,
 		   list2 (Qstringp, build_ascstring ("type")));
#endif	/* SXEMACS */
        }
  ...
  else if (CONSP (val_type) && EQ (XCAR (val_type), Q_struct))
    {
      ...
      memcpy ((char *) ptr, (const char *) XEFFIO (val)->fop.ptr,
              XFIXNUM (Fffi_size_of_type (val_type)));
    }
  else
    {
#ifdef SXEMACS
      ...
      Fsignal(Qinternal_error,
              list2(Qstringp,
                    build_ascstring("non basic or pointer type")));
#endif	/* SXEMACS */
    }
...
extffi_setup_argument(Lisp_Object type, ffi_type **ft)
{
  ...
  if (EQ(type, Q_char) || EQ(type, Q_byte))
    ...
    *ft = &ffi_type_pointer;
    ...
    nt_size = XFIXNUM(Flength(slots)) + 1;
    if (nt_size + lf_cindex > MAX_TYPES_VALUES) {
      ...
      Fsignal(Qoverflow_error,
	      list2(Qstringp,
		    build_ascstring("cindex overflow")));
#endif	/* SXEMACS */
    }
      ...
      (int nargs, Lisp_Object *args))
{
  ...
  fof_nargs = XFIXNUM(Flength(XCDR(XEFFIO(fof)->type)))-1;
  fof_args = &args[1];
...
ffi_callback_call_x86 (Lisp_Object cbk_info, char *arg_buffer)
{
  ...
  while (!NILP (argtypes))
    {
      ...
      size = XFIXNUM (Fffi_size_of_type (ctype));
  ...
  Fffi_store (foret, make_fixnum (0), rtype, retlo);
...
ffi_make_callback_x86 (Lisp_Object data, int cc_type)
{
  ...
  *(char *) (buf + 12) = 0x59;
  if (cc_type == FFI_CC_CDECL)
	...
	{
	  int sz = XFIXNUM (Fffi_size_of_type (XCAR (arg_types)));
       ...
       (fun, rtype, argtypes, cctype))
{
  ...
  ptr = Fmake_ffi_object (Q_pointer, Qnil);
#ifdef __i386__
  XEFFIO (ptr)->fop.ptr = ffi_make_callback_x86 (data, XFIXNUM (cctype));
#endif /* __i386__ */
  return ptr;
}
