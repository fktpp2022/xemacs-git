		      ...
		      int *debugger_entered)
{
  ...
  if (!entering_debugger && !*debugger_entered && !signal_vars_only
      ...
      && !skip_debugger (conditions, temp_data))
    {
      ...
      /* This will call #'early-error-handler and die if we are
         noninteractive. */
...
*/ );
  ...
  dump_mark_nil_lisp_object (&Vstack_trace_on_error);
...
*/ );
  ...
  dump_mark_nil_lisp_object (&Vdebug_on_error);
...
*/ );
  ...
  dump_mark_nil_lisp_object (&Vdebug_on_signal);
